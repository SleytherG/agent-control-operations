...
class ClosingAnnulledOpsTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_annulled_operations_listed_but_excluded_from_metrics(): void
    {
        Operation::create([
        ...
        Operation::create([
            'organization_id' => $this->org->id, 'store_id' => $this->agent->store_id,
            'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->type->id,
            'user_id' => $this->admin->id, 'amount' => 999.00, 'currency' => 'PEN',
            'effective_at' => $this->businessDate . ' 09:00:00', 'recorded_at' => now(),
            'status' => Operation::STATUS_ANNULLED, 'annulled_by' => $this->admin->id,
            'annulled_at' => now(), 'annulment_reason' => 'Error en monto',
            'idempotency_key' => \Illuminate\Support\Str::random(64),
            'created_at' => now(), 'updated_at' => now(),
        ]);
        ...
        $response->assertOk();
        $response->assertSee('Operaciones Anuladas del Periodo');
        $response->assertSee('Error en monto');
    }

    public function test_agent_with_only_annulled_operations_shows_zeros(): void
    {
        Operation::create([
            'organization_id' => $this->org->id, 'store_id' => $this->agent->store_id,
            'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->type->id,
            'user_id' => $this->admin->id, 'amount' => 500.00, 'currency' => 'PEN',
            'effective_at' => $this->businessDate . ' 10:00:00', 'recorded_at' => now(),
            'status' => Operation::STATUS_ANNULLED, 'annulled_by' => $this->admin->id,
