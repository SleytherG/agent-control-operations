...
class ReopenAllowsAnnulTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_annulment_allowed_after_reopen(): void
    {
        ...
        $this->post(route('daily-closures.reopen', $closure), [
            'reason' => 'Corrección necesaria',
        ]);

        $this->actingAsJwt($this->operator);

        $response = $this->post(route('operations.annul', $operation), [
            'reason' => 'Anulación después de reapertura',
        ]);
        ...
        $operation->refresh();
        $this->assertEquals(Operation::STATUS_ANNULLED, $operation->status);
    }
}
