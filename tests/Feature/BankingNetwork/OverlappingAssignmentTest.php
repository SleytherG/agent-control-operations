...
class OverlappingAssignmentTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_cannot_assign_same_agent_twice_to_same_operator(): void
    {
    ...
    public function test_can_reassign_after_previous_is_inactive(): void
    {
        $this->actingAsJwt($this->admin);

        $old = UserBankAgentAssignment::create([
            'user_id' => $this->operator->id,
            'bank_agent_id' => $this->agent->id,
            'assigned_by' => $this->admin->id,
            'assigned_at' => now(),
