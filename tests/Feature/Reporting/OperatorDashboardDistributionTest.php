...
class OperatorDashboardDistributionTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_type_distribution_shows_all_operation_types(): void
    {
        Operation::create([
            'organization_id' => $this->org->id, 'store_id' => $this->agent->store_id,
            'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->entryType->id,
            'user_id' => $this->operator->id, 'amount' => 500.00, 'currency' => 'PEN',
        ...
        Operation::create([
            'organization_id' => $this->org->id, 'store_id' => $this->agent->store_id,
            'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->exitType->id,
            'user_id' => $this->operator->id, 'amount' => 200.00, 'currency' => 'PEN',
            'effective_at' => now()->subHour()->format('Y-m-d H:i:s'),
            'recorded_at' => now(), 'status' => 'ACTIVE',
            'idempotency_key' => Str::random(64), 'created_at' => now(), 'updated_at' => now(),
        ]);
        ...
        $response->assertSee('Distribución por tipo de operación');
        $response->assertSee('Depósito');
        $response->assertSee('Retiro');
    }

    public function test_type_distribution_empty_when_no_operations(): void
    {
        $this->actingAsJwt($this->operator);
