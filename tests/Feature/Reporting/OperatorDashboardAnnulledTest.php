...
class OperatorDashboardAnnulledTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_annulled_operations_excluded_from_metrics(): void
    {
        Operation::create([
        ...
        Operation::create([
            ...
            'annulled_by' => $this->operator->id, 'annulled_at' => now(),
    ...
    public function test_annulled_operations_not_counted_in_count(): void
    {
        Operation::create([
            'organization_id' => $this->org->id, 'store_id' => $this->agent->store_id,
            'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->entryType->id,
            'user_id' => $this->operator->id, 'amount' => 10.00, 'currency' => 'PEN',
            'effective_at' => now()->subHour()->format('Y-m-d H:i:s'),
            'recorded_at' => now(), 'status' => 'ACTIVE',
            'idempotency_key' => Str::random(64), 'created_at' => now(), 'updated_at' => now(),
        ]);
        ...
        Operation::create([
            'organization_id' => $this->org->id, 'store_id' => $this->agent->store_id,
            'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->entryType->id,
            'user_id' => $this->operator->id, 'amount' => 20.00, 'currency' => 'PEN',
            'effective_at' => now()->subHour()->format('Y-m-d H:i:s'),
            'recorded_at' => now(), 'status' => 'ANNULLED',
            'annulled_by' => $this->operator->id, 'annulled_at' => now(),
            'annulment_reason' => 'Error', 'idempotency_key' => Str::random(64),
        ...
        $response->assertStatus(200);
        $response->assertSee('S/ 10.00');
        $response->assertDontSee('S/ 20.00');
    }
}
