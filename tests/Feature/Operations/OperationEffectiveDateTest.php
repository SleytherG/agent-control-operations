...
class OperationEffectiveDateTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_date_beyond_retroactive_window_is_rejected(): void
    {
        ...
        $response = $this->post(route('operations.store'), [
            ...
            'amount' => 100.00,
            'effective_at' => now()->subHours(25)->format('Y-m-d H:i:s'),
            'idempotency_key' => Str::random(64),
        ]);

        $response->assertSessionHasErrors('effective_at');
