...
class OperationAnnulledVisibilityTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        Operation::create(['organization_id' => $this->org->id, 'store_id' => $store->id, 'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->type->id, 'user_id' => $this->operator->id, 'amount' => 100.00, 'currency' => 'PEN', 'effective_at' => now(), 'recorded_at' => now(), 'status' => 'ACTIVE', 'idempotency_key' => hash('sha256', 'active1'), 'created_at' => now(), 'updated_at' => now()]);
        Operation::create(['organization_id' => $this->org->id, 'store_id' => $store->id, 'bank_agent_id' => $this->agent->id, 'operation_type_id' => $this->type->id, 'user_id' => $this->operator->id, 'amount' => 200.00, 'currency' => 'PEN', 'effective_at' => now(), 'recorded_at' => now(), 'status' => 'ANNULLED', 'annulled_by' => $admin->id, 'annulled_at' => now(), 'annulment_reason' => 'Error', 'idempotency_key' => hash('sha256', 'annulled1'), 'created_at' => now(), 'updated_at' => now()]);
    }

    public function test_operator_sees_annulled_operations_in_history(): void
    {
